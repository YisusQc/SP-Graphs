...
public:
  ...
  void loadNodes(const std::string& nodes_file);
  std::vector<long long> reconstruct_path(const std::unordered_map<long long, long long>& prev, long long start, long long goal) const;
